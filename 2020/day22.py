...
def play_combat(deck1, deck2, recursive=False):
    ...
    while deck1 and deck2:
        ...
        if recursive and len(deck1) >= c1 and len(deck2) >= c2:
            r1, r2 = play_combat(deque(list(deck1)[:c1]), deque(list(deck2)[:c2]))
...
def solve(inputs):
    ...
    print(f"Part 1: {calculate_score(*play_combat(deck1, deck2, False))}")

    deck1, deck2 = (deque(map(int, x.splitlines()[1:])) for x in inputs.split("\n\n"))
    print(f"Part 2: {calculate_score(*play_combat(deck1, deck2, True))}\n")
