...
class ConnectedGraph(ABC):
    ...
    def shortest_path(self, start, goals, check_closest_neighbour=True):
        ...
        for node, (distance, _) in paths_to_goals.items():
            ...
            if distance > shortest_distance:
                continue
            if distance < shortest_distance or node < closest_goal:
                closest_goal = node
                shortest_distance = distance
        ...
        if check_closest_neighbour:
            ...
            for nb in self.connected_nodes(start):
                ...
                if len(nb_path) > len(best_nb_path):
                    continue
                if len(nb_path) < len(best_nb_path) or nb < best_nb_path[0]:
                    best_nb_path = nb_path
            path = best_nb_path
        else:
            path = [closest_goal]
...
class Unit:
    ...
    @property
    def order_key(self):
        return self.loc.y * 1000 + self.loc.x
...
class CaveSystem(ConnectedGraph):
    ...
    def in_range_target(self, unit, targets):
        ...
        if not in_range_targets:
            return None
        return sorted(in_range_targets, key=lambda u: u.hp)[0]
    ...
    def print_map(self):
        for y in range(self.max_y):
            ...
            for x in range(self.max_x):
                row += self.map[(x, y)]
            row_units = [
                f"{u.species}({u.hp})" for u in sorted(self.survivors()) if u.loc.y == y
            ]
            row += "  " + ", ".join(row_units)
            print(row)
        print()
