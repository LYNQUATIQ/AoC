...
class UndergroundCavern(ConnectedGrid):
        ...
        for start in range(4):
            ...
            for xy, key in self.keys.items():
                if start == 0:
                    ...
                    in_quadrant = xy.x > start_xy.x and xy.y > start_xy.y
                if in_quadrant:
                    self.quadrant_keys[start].append(key)
        print(self.quadrant_keys)
    ...
    def gather_all_paths(self):
        ...
        for start in range(4):
            start_paths = self.paths_to_goals(self.start[start], self.keys.keys())
            for key, path in start_paths.items():
                store_in_cache(start, self.keys[key], path)
