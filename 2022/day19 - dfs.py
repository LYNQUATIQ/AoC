...
def print_path(blueprint, end_states, prior_states, max_time) -> None:

    max_geodes = max(end_states.values())
    max_states = [
        s for s, g in end_states.items() if s[0] == max_time and g == max_geodes
    ]
    path = [max_states[0]]
    s = path[0]
    while True:
...
def print_state(blueprint, state: State) -> None:
